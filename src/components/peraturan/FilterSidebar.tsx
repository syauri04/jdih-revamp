    ...
    <>
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6">
          <h3 className="font-bold text-2xl text-[#043927]">
          ...
          <input
            ...
            className="w-full h-12 px-4 border bg-white border-black/10 rounded-[10px]"
          />

          <div className="space-y-3">
            {[
              { label: "Pedoman", value: "pedoman", count: 12 },
              { label: "Undang - Undang", value: "uu", count: 12 },
              { label: "Peraturan Pemerintah", value: "pp", count: 23 },
              { label: "Peraturan Presiden", value: "perpres", count: 34 },
              {
                label: "Peraturan Menteri Pertanian",
                value: "permentan",
                count: 56,
              },
              { label: "Instruksi Presiden", value: "inpres", count: 26 },
              { label: "Keputusan Menteri", value: "kepmen", count: 26 },
            ].map((item) => (
              <label
                key={item.value}
        ...
        {/* TEMATIK */}
        <div className="space-y-6">
          <h3 className="font-bold text-2xl text-[#043927]">Tematik</h3>

          <div className="space-y-3">
            {[
              {
                label: "Manajemen dan Kesekretariatan",
                value: "Manajemen-dan-Kesekretariatan",
                count: 12,
              },
              {
                label: "Prasarana dan Sarana",
                value: "Prasarana-dan-Sarana",
                count: 12,
              },
              { label: "Tanaman Pangan", value: "Tanaman-Pangan", count: 23 },
              { label: "Hortikultura", value: "Hortikultura", count: 34 },
              {
                label: "Peternakan dan Kesehatan Hewan",
                value: "Peternakan-dan-Kesehatan-Hewan",
                count: 56,
              },
              { label: "Perkebunan", value: "Perkebunan", count: 26 },
              {
                label: "Penelitian dan Pengembangan",
                value: "Penelitian-dan-Pengembangan",
                count: 26,
              },
              {
                label: "Penyuluhan dan SDM Pertanian",
                value: "Penyuluhan-dan-SDM-Pertanian",
                count: 26,
              },
              {
              ...
              >
                <div className="flex items-center gap-3">
                  <input
                    type="checkbox"
                    checked={filters.tematik.includes(item.value)}
                    onChange={() => toggle("tematik", item.value)}
                  />
                  <span className="text-[#043927]">{item.label}</span>
                </div>
        ...
        {/* STATUS */}
        <div className="space-y-6">
          <h3 className="font-bold text-2xl text-[#043927]">Status</h3>

          {["Berlaku", "Tidak Berlaku"].map((status) => (
            <label
              key={status}
              className="flex items-center justify-between cursor-pointer"
            >
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-[#043927]">{status}</span>
              </div>
            </label>
          ))}
        </div>
